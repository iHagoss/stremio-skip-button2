...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        timeNowTextView = playerView.findViewById(R.id.timeNow)
        timeEndTextView = playerView.findViewById(R.id.timeEnd)
    ...
    private fun createSkipRangeManager(): SkipRangeManager {
        ...
        val traktApiKey = prefs.getString(ENV_TRAKT_API_KEY, null) ?: System.getenv(ENV_TRAKT_API_KEY)
        val tmdbApiKey = prefs.getString(ENV_TMDB_API_KEY, null) ?: System.getenv(ENV_TMDB_API_KEY)
        val tvdbApiKey = prefs.getString(ENV_TVDB_API_KEY, null) ?: System.getenv(ENV_TVDB_API_KEY)
    ...
    private fun updateTimeDisplays(player: ExoPlayer) {
        try {
            val now = Calendar.getInstance()
            timeNowTextView?.text = "Now: ${timeFormat.format(now.time)}"
            ...
            if (duration > 0) {
                ...
                val adjustedRemaining = (remaining / playbackSpeed).toLong()
                
                val endTime = Calendar.getInstance()
                endTime.add(Calendar.MILLISECOND, adjustedRemaining.toInt())
                timeEndTextView?.text = "Ends: ${timeFormat.format(endTime.time)}"
            } else {
                timeEndTextView?.text = "Ends: --:-- --"
            }
        } catch (e: Exception) {
            Log.w(TAG, "Error updating time displays", e)
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.action_manual_skip -> {
                val intent = Intent(this, ManualSkipEditorActivity::class.java)
                startActivity(intent)
                true
            }
